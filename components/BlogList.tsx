      ...
      >
        {orderedPosts.map((post) => (
          <motion.article key={post.slug} variants={staggerItem} className="group">
            <Link
              href={`/blog/${post.slug}`}
              className="flex h-full flex-col gap-10 rounded-[40px] px-4 pb-4 pt-12 md:gap-12 md:rounded-[56px] md:pt-14"
            ...
            >
              ...
              {/* Bas : image en cover, très arrondie */}
              <div className="overflow-hidden rounded-[28px] md:rounded-[48px]">
                {post.image ? (
                  <Media
                    src={post.image}
                    alt={post.title}
